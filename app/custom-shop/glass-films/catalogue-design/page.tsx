...
const filteredDesigns = designs.filter((design) => {
  ...
  useEffect(() => {
  const loadGlassFilmCatalogues = async () => {
    try {
      const category = await dispatch(getCategoryBySlug("glass-films"))
      if (!category?._id) return

      const res = await dispatch(getCataloguesByCategory(category._id))
      setDesigns(res.map(adaptCatalogue))
    } catch (err) {
      console.error("Failed to load glass film catalogue", err)
    } finally {
      setLoading(false)
    }
  }

  loadGlassFilmCatalogues()
}, [dispatch])


if (loading) {
  return <div className="p-10 text-center">Loading designs...</div>
}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              <Link href="/custom-shop/glass-films">
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link href={`/custom-shop/glass-films/catalogue-design/${design.id}`}>
