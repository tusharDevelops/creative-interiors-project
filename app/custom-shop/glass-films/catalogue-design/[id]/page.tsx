...
export default function GlassFilmCatalogueCustomization() {
...
useEffect(() => {
  const loadCatalogue = async () => {
    try {
      ...
      console.error("Failed to load glass film design", err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const area = (Number(width) * Number(height)) / 144

  const stylePrice =
    filmStyles.find((s) => s.value === filmStyle)?.price || 0

  const tintPrice =
    colorTints.find((c) => c.value === colorTint)?.price || 0

  const baseTotal = (basePrice + stylePrice + tintPrice) * area * quantity
  ...
  const configurationJson = JSON.stringify({
    ...
    category: "GLASS_FILM_CATALOGUE",
    specialInstructions,
  })

  await dispatch(
    addToCart({
      productType: "GLASS_FILM",
  ...
  if (loading) {
  return <div className="p-10 text-center">Loading design...</div>
}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              <Link href="/custom-shop/glass-films/catalogue-design">
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  {design?.features?.map((feature:string, index:number):any => (
