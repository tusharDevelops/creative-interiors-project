...
export function TeamAccordion({ items }: { items: TeamMember[] }) {
  return (
    <div className="group flex max-md:flex-col justify-center gap-2 w-[50%] mx-auto mb-10 mt-3">
      {items.map((item) => (
        <article
          key={item.id}
        ...
        >
          <a className="absolute inset-0 text-black z-10 p-3 flex flex-col justify-end" href="#0">
          ...
          <Image
  className="object-cover h-72 md:h-[420px] w-full filter grayscale group-hover/article:grayscale-0 transition-all duration-500 ease-in-out"
